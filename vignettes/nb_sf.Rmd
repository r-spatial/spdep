...
```{r, echo=dothis, eval=dothis}
system.time(pts_ll3_nb <- dnearneigh(pts_ll, d1=0, d2=0.75*max_1nn, avoid_s2=FALSE))
```
Time can be saved by reducing the cell count used to approximate the buffer:

```{r, echo=dothis, eval=dothis}
system.time(pts_ll4_nb <- dnearneigh(pts_ll, d1=0, d2=0.75*max_1nn, avoid_s2=FALSE, max_cells=500))
```
...
```{r, echo=dothis, eval=dothis}
system.time(for (i in 1:(reps/5)) pts_ll5_nb <- dnearneigh(pts_ll, d1=0, d2=0.75*max_1nn, avoid_s2=FALSE, dwithin=TRUE))/(reps/5)
```
