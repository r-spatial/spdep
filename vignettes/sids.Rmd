...
nc_sf <- st_read(system.file("shape/nc.shp", package="sf"),
                 quiet=TRUE)
st_crs(nc_sf)
spData_sf <- st_read(system.file("shapes/sids.shp",
...
```{r echo=TRUE,eval=TRUE}
st_crs(spData_sf) <- "+proj=longlat +datum=NAD27"
```
...
if (require("rgeos", quietly=TRUE)){
  gEquals(as(st_geometry(spData_sf), "Spatial"),
          as(st_geometry(nc_sf), "Spatial"))
}
```
...
```{r echo=TRUE,eval=TRUE}
if (require("rgeos", quietly=TRUE)){
  gEquals(as(st_geometry(sids_sf), "Spatial"),
          as(st_geometry(nc_sf), "Spatial"))
}
```
...
```{r echo=TRUE,eval=TRUE}
if (require("rgeos", quietly=TRUE)){
  gEquals(as(st_geometry(sids2_sf), "Spatial"),
          as(st_geometry(nc_sf), "Spatial"))
}
```

Now for the contents of the files - `sids2` also contains rates, while `spData` contains the coordinates given in @cressie:1991, and the parcels of contiguous counties on p. 554, and the aggregations used for median polishing.
...
```{r echo=TRUE, eval=TRUE}
all.equal(as.data.frame(nc_sf)[,1:14], as.data.frame(spData_sf)[,c(2,3,4,1,5:14)])
```

so a difference in `NWBIR74`:

```{r echo=TRUE, eval=TRUE}
which(!(nc_sf$NWBIR74 == spData_sf$NWBIR74))
c(spData_sf$NWBIR74[21], nc_sf$NWBIR74[21])
```
