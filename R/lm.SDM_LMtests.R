...
lm.SDM_LMtests <- function(model, listw, zero.policy=attr(listw, "zero.policy"), test="all", spChk=NULL, naSubset=TRUE, Durbin=TRUE, alternative="greater") {
...
# fixed after looking at TOWN dummy in Boston data
	...
	for (i in 1:nt) {
		...
		SDM_LMWX = vec <- c((1/N) * t(dg) %*% invJg.p %*% dg, ncol(X0)),
